let textcontent: HTMLTextAreaElement;
...
window.addEventListener('load', () => {
    ...
    const valueListener = () => {
        ...
        } else send.classList.remove('ready')
    }
    const sendListener = () => {
        let v = textcontent.value;
        v = (v || '').trim();
...
function incomingMessage (name: string, msg: string, id: string) {
...
function outgoingMessage (name: string, msg: string, id: string) {
    let message_line = document.createElement('div');
...
function systemMessage (info: string) {
    let message_line = document.createElement('div');
...
function scrollToChatBottom () {
    ...
    if (childrenHeight > chatbox.getBoundingClientRect().height && chatbox.scrollTop < childrenHeight) {
        ...
        let scrollBy = .75;
        let scrolled = 0;
        let animationFrameValues = new Array<number>(0);
        ...
        let scrollAnimation = (index: number) => {
